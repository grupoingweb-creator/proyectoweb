...
function Sidebar({ onLogout }) { // <-- Recibe la función onLogout
  const [open, setOpen] = useState(true);
  const location = useLocation();

  const menuItems = [
    { path: "/dashboard", label: "Dashboard", icon: <BarChart2 /> },
    { path: "/ventas", label: "Historial Ventas", icon: <ShoppingCart /> },
    { path: "/productos", label: "Productos", icon: <Box /> },
    { path: "/clientes", label: "Usuarios", icon: <Users /> },
    { path: "/proveedores", label: "Proveedores", icon: <Truck /> },
    { path: "/compras", label: "Compras", icon: <ClipboardList /> },
    { path: "/reportes", label: "Reportes", icon: <LineChart /> },
      ...
      <div className="flex-1 flex flex-col justify-between">
        ...
        <div className="mt-10 border-t border-gray-600 pt-4">
          <button
            ...
            className="flex items-center gap-3 p-2 rounded-lg hover:bg-red-600 transition-all duration-300 hover:scale-105 w-full"
          >
            <LogOut />
